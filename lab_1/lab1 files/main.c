...
void FixedExperiment(void) {
	int16_t a = 16384;   // should be 0.5; + 0.100000000000000 = 16384 in base 10
	int16_t b = 4096; // should be 0.125; 0.001000000000000 = 4096 in base 10
	int32_t temp = ((int32_t)a * (int32_t)b);
	int16_t c = temp>>15; // leave this as is initially
}

void CMSISExperiment(void) {
	float32_t floatArray[10] = {0.5, -0.3, 1.2, -1.1, 0.99, -0.99, 0.75, -0.45, 1.5, -1.5};
    q15_t q15Array[10];
    float32_t convertedFloatArray[10];


    //arm_float_to_q15 (const float32_t *pSrc, q15_t *pDst, uint32_t blockSize)
    arm_float_to_q15(floatArray, q15Array, 10);

    arm_q15_to_float(q15Array, convertedFloatArray, 10);
}
